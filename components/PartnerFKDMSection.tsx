...
import { FKDM } from '../types';

const PartnerFKDMSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await apiService.getFKDM();
      if (data) setFkdmList(data);
    };
    fetchData();
  }, []);

  const filteredFKDM = useMemo(() => {
    return fkdmList.filter(l => 
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Optimistic Update
    setFkdmList(prev => [...prev, newEntry]);
    
    // API Call
    await apiService.createFKDM(newEntry);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-red-700 to-rose-800 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10">
           <h2 className="text-3xl font-black mb-2">Forum Kewaspadaan Dini Masyarakat (FKDM)</h2>
           <p className="text-red-100 max-w-xl">
             Mitra strategis dalam menjaga stabilitas, keamanan, dan ketertiban masyarakat di lingkungan kelurahan.
           </p>
        </div>
        <ShieldCheck className="absolute right-0 bottom-0 text-white opacity-10 w-48 h-48 -mr-10 -mb-10" />
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
         <div className="flex items-center gap-2 text-slate-500 text-sm font-bold">
            <Users size={18} />
            <span>Total Anggota: {fkdmList.length}</span>
         </div>
         <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-red-500 outline-none transition-all"
                />
            </div>
            <button 
                onClick={() => setIsAddModalOpen(true)}
                className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-red-200 shrink-0"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {filteredFKDM.map((member) => (
            <div key={member.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-lg transition-all group relative overflow-hidden">
               <div className="absolute top-0 right-0 bg-red-50 text-red-600 px-4 py-2 rounded-bl-2xl font-black text-xs uppercase tracking-wide">
                  {member.jabatan}
               </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-200">
             <h3 className="text-xl font-bold text-slate-800 mb-6">Tambah Anggota FKDM</h3>
             <form onSubmit={handleSave} className="space-y-4">
                <div className="space-y-1">
                   ...
                   <input 
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-500"
                ...
                <div className="space-y-1">
                   ...
                   <input 
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-red-500 font-mono"
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-800 text-white w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-in zoom-in-95 border border-slate-700">
             ...
             <p className="text-slate-300 mb-6">Data Anggota FKDM Berhasil Ditambahkan!</p>
...
export default PartnerFKDMSection;
