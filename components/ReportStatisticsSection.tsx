...
const ReportStatisticsSection: React.FC<ReportStatisticsSectionProps> = ({ reports, setReports, staffList, setStaffList }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="min-h-[500px]">
        {activeSlide === 1 ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-700 fade-in">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm"><h4 className="font-bold text-slate-800 text-sm mb-6 flex items-center gap-2 uppercase tracking-wider"><Trophy size={16} className="text-yellow-500" /> Peringkat Wilayah (RW) Teraktif</h4><div className="h-[350px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={rwStats} layout="vertical" margin={{ left: 40, right: 20 }}><CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#f1f5f9" /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={80} axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 700, fill: '#475569'}} /><Tooltip contentStyle={{ borderRadius: '16px' }} /><Bar dataKey="count" name="Jumlah" fill="#3b82f6" radius={[0, 6, 6, 0]} barSize={24} /></BarChart></ResponsiveContainer></div></div>
      ...
      {detailReport && <ReportDetailModal report={detailReport} staffList={staffList} onClose={() => setDetailReport(null)} onAction={handleDetailAction} />}
